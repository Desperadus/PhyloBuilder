    ...
    **kwargs,
    ...
    for species in tqdm(species_list):
        search_query = construct_search_query(
            species, gene_name, min_length, max_length, prompt_template, template_values
        )
        sequence_id, fasta_sequence = fetch_sequence(
            search_query, skip_errors, **kwargs
        )

        if sequence_id and fasta_sequence:
            fasta_sequences.append(fasta_sequence)
            report_sequence_found(
                species, gene_name, sequence_id, fasta_sequence, verbose
            )
            found_species.append(
                (species, fasta_sequence.splitlines()[0].split(" ")[0][1:])
            )
            ...
            handle_not_found(
                species, gene_name, unfound_species, fasta_sequences, skip_errors
            )
...
def fetch_sequence(
    search_query: str, skip_errors: bool, **kwargs
    ...
    try:
        search_handle = Entrez.esearch(
            db="nucleotide", term=search_query, sort="relevance", **kwargs
        )
...
def report_sequence_found(
    species: str, gene_name: str, sequence_id: str, fasta_sequence: str, verbose: bool
) -> None:
    """
    ...
    if verbose:
        print(
            f"For {species} {gene_name}, found {sequence_id} with length {len(fasta_sequence)}."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch gene sequences for a list of species."
    )
    parser.add_argument(
        "-f",
        "--file",
    ...
    parser.add_argument(
        ...
        help="Output file to save the fetched sequences.",
    )
    parser.add_argument(
        "-g", "--gene", required=True, help="Gene name to fetch sequences for."
    )
    parser.add_argument(
        "-e",
        "--email",
        required=True,
        help="Email address to use with the Entrez API for tracking purposes.",
    )
    parser.add_argument(
        "-min",
        "--min_length",
        type=int,
        default=0,
        help="Minimum length of the gene sequences to be fetched. Defaults to 0.",
    )
    parser.add_argument(
        "-max",
        "--max_length",
        type=int,
        default=1000000,
        help="Maximum length of the gene sequences to be fetched. Defaults to 1_000_000.",
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="If set, prints additional information.",
    )
    parser.add_argument(
        "-se",
        "--skip_errors",
        action="store_true",
        help="If set, skips adding sequences that could not be obtained.",
    )
    parser.add_argument(
        "-sw",
        "--skip_warnings",
        action="store_true",
        help="If set, skips adding sequences where species name does not match header.",
    )
